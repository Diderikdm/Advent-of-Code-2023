...
with open("day19.txt", "r") as file:
    ...
    for row in data:
        ...
        for check in checks:
            if ":" not in check: 
                other_label, current = check, [[vars[x], vars[x.upper()]] for x in "xmas"]
            else:
                ...
                for x in "xmas":
                    if x == var:
                        if sign == ">":
                            current += [[int(value) + 1, vars[x.upper()]]]
                            vars[x.upper()] = int(value)
                        elif sign == "<":
                            current += [[vars[x], int(value) - 1]]
                            vars[x] = int(value)
                    else:
                        current += [[vars[x], vars[x.upper()]]]                
